...
class CryptoPositionsBot {
    ...
    async getUserInfo(twitchChannel) {
        ...
        if (user) {
            user = user[1];
            user.EXCHANGE_KEYS = user.EXCHANGE_KEYS.map(key => ({ EXCHANGE_ID: key.EXCHANGE_ID, API_KEY: new Array(key.API_KEY.length + 1).join("*"), API_SECRET: new Array(key.API_SECRET.length + 1).join("*"), DESCRIPTION: key.DESCRIPTION }));
            return user;
        }
        else {
            try {
