...
class BybitPositionBot {
            ...
            if (user !== undefined && user.TWITCH_ENABLED) {
                if (chatter.message === '!position' || chatter.message === '!positions') {
                    ...
                    if (!this.twitchCommandTimeouts['position'][chatter.channel].timedout) {
                        ...
                        if (positions) {
                            ...
                            if (formattedMessage.length > 1) {
                                formattedMessage.forEach((message, index) => {
                                    setTimeout(() => this.twitchBot.say(message, chatter.channel), index * 2000);
                                });
                            }
                            else {
                        ...
                        this.twitchBot.say(`Please try again in ${Math.floor((this.twitchCommandTimeouts['position'][chatter.channel].expires - Date.now()) / 1000)} seconds.`, chatter.channel);
                    }
                }
            }
        });
    ...
    getSignature(userID, timestamp, recvWindow, data) {
        let user = users.get(userID);
    ...
    async bybitAuthenticatedHttpRequest(userID, endpoint, method, data) {
        let user = users.get(userID);
    ...
    async fetchPositionsAndOrders(userID) {
        let user = users.get(userID);
        if (user) {
    ...
    //     });
    // }
    async addUser(bybit_api_key, bybit_api_secret, discord_channel_id, twitch_channel) {
...
exports.default = BybitPositionBot;
// const bot = new BybitPositionBot();
// bot.start();
//# sourceMappingURL=BybitPositionBot.js.map