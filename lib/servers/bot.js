...
//     }
// });
bot.addUser([{ 'KEY_ID': '30ba7142-3c04-4720-9577-8b1c656bb6ab', 'API_KEY': 'Dfv0YiiuJuz7Y5aIp1', 'API_SECRET': '3teMGGWmcARkm5y9ryOehwx6mWqqqQm6J37H', 'EXCHANGE_ID': 'bybit', 'DESCRIPTION': 'Challange Account' }], '1044690741424836732', '#bearkingeth', true, ["pnl", 'liq', 'entry', 'size', 'mark'], true, true, 5, ["pnl", 'liq', 'entry', 'size', 'mark'], true).then((userID) => {
    if (userID !== null) {
        bot.enableUser(userID);
        bot.connectToTwitchChannel(userID);
    }
...
const getChannelPositions = (access_token, is_helix = 'false', client_id, channel_id) => {
    return new Promise((resolve, reject) => {
        axios_1.default.get(`https://api.twitch.tv/helix/users${channel_id !== 'undefined' && channel_id !== undefined ? '?id=' + channel_id : ''}`, {
            headers: { "Authorization": `${(is_helix === 'true' || is_helix === true) ? 'Extension' : 'Bearer'} ${access_token}`,
                "Client-Id": (client_id !== 'undefined' && client_id !== undefined) ? client_id : process.env.TWITCH_APP_CLIENT_ID }
        }).then((response) => {
            resolve(bot.getChannelPositions(response.data.display_name));
        }).catch(error => {
            console.error(error);
            reject(error);
        });
...
app.get('/getChannelPositions', (0, cors_1.default)(), (req, res) => {
    if (req.query !== undefined) {
        if (req.query.access_token === undefined) {
            res.send('missing access_token');
        }
        else {
            getChannelPositions(req.query.access_token, req.query.is_helix, req.query.client_id, req.query.channel_id).then((data) => {
                res.send(data);
            }).catch(error => {
                console.error(error);
                res.status(500).send(error);
...
app.get('/discord-bot-invite', (0, cors_1.default)(), (req, res) => {
